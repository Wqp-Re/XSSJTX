...
panel.setContentSize(680, 640);
panel.setPosition(0, 0);
var g = panel.addComponent(cc.Graphics);
g.fillColor = new cc.Color(15, 15, 30, 240);
g.rect(-340, -320, 680, 640);
g.fill();
...
tlb.color = cc.Color.YELLOW;
tlb.lineHeight = 42;
title.setPosition(0, 290);
panel.addChild(title);
// 营地升级按钮
var cupbtn = new cc.Node("campup");
cupbtn.setContentSize(260, 56);
var cb2 = cupbtn.addComponent(cc.Graphics);
cb2.fillColor = new cc.Color(40, 80, 160);
cb2.rect(-130, -28, 260, 56);
cb2.fill();
var culb = new cc.Node("culb");
var cul = culb.addComponent(cc.Label);
cul.fontSize = 22;
cul.string = "营地升级(" + cc.getCampUpCost() + "金)";
cul.color = cc.Color.WHITE;
cul.isSystemFontUsed = !0;
culb.setPosition(0, 0);
cupbtn.addChild(culb);
cupbtn.setPosition(0, 235);
panel.addChild(cupbtn);
cupbtn.on(cc.Node.EventType.TOUCH_END, function() {
var r = cc.upgradeCamp();
if (0 == r) {
cc.uiHelper.showTips("营地升级成功!当前Lv." + cc.getCampLv());
_this._campPanel.destroy(); _this._campPanel = null; _this.createCamp();
} else if (2 == r) cc.uiHelper.showTips("营地已达最高等级");
else if (3 == r) cc.uiHelper.showTips("金币不足");
});
// 设施列表
...
(function(k, ypos) {
var row = new cc.Node("fac" + k);
row.setContentSize(600, 78);
var rg = row.addComponent(cc.Graphics);
rg.fillColor = new cc.Color(30, 30, 45, 220);
rg.rect(-300, -39, 600, 78);
rg.fill();
var f = fm[k];
...
var lb = lbn.addComponent(cc.Label);
lb.fontSize = 22;
lb.isSystemFontUsed = !0;
lb.color = cc.Color.WHITE;
lb.overflow = cc.Label.Overflow.SHRINK;
lbn.setContentSize(360, 60);
lb.string = f.name + " Lv." + lv + "/" + facMax + "\n" + f.des;
lbn.setPosition(-80, 0);
row.addChild(lbn);
// 升级按钮
var fup = new cc.Node("fup");
fup.setContentSize(150, 50);
var fbg = fup.addComponent(cc.Graphics);
fbg.fillColor = new cc.Color(60, 110, 60);
fbg.rect(-75, -25, 150, 50);
fbg.fill();
var flb2 = new cc.Node("flb");
var fl = flb2.addComponent(cc.Label);
fl.fontSize = 18;
fl.string = "升级(" + cc.getFacUpCost(k) + "金)";
fl.color = cc.Color.WHITE;
fl.isSystemFontUsed = !0;
fl.overflow = cc.Label.Overflow.SHRINK;
flb2.setContentSize(140, 40);
flb2.setPosition(0, 0);
fup.addChild(flb2);
fup.setPosition(190, 0);
row.addChild(fup);
fup.on(cc.Node.EventType.TOUCH_END, function() {
var r = cc.upgradeFac(k);
if (0 == r) {
cc.uiHelper.showTips(f.name + "升级成功!");
_this._campPanel.destroy(); _this._campPanel = null; _this.createCamp();
} else if (2 == r) cc.uiHelper.showTips("设施已达当前营地上限(升营地可提升)");
else if (3 == r) cc.uiHelper.showTips("金币不足");
});
row.setPosition(0, ypos);
panel.addChild(row);
})(keys[i], 170 - i * 84);
}
// 关闭按钮
...
var cg = close.addComponent(cc.Graphics);
cg.fillColor = new cc.Color(120, 30, 30);
cg.rect(-100, -28, 200, 56);
cg.fill();
...
cl.isSystemFontUsed = !0;
clb.setPosition(0, 0);
close.addChild(clb);
close.setPosition(0, -288);
panel.addChild(close);
close.on(cc.Node.EventType.TOUCH_END, function() {
...
cc.campCfg = {
lvMax: 10,
lvCost: [ 0, 5000, 20000, 8e4, 2e5, 5e5, 1e6, 2e6, 5e6, 1e7 ],
facMax: 10,
facs: {
pet: { name: "宠物屋", des: "宠物全成长+1/级", costBase: 1e3, mul: 1 },
forge: { name: "铁匠铺", des: "爆伤+10/级", costBase: 1500, mul: 1.5 },
pond: { name: "钓鱼塘", des: "生命+200/级", costBase: 800, mul: 1 },
train: { name: "训练场", des: "攻击+20/级", costBase: 1200, mul: 1.2 },
store: { name: "仓库", des: "防御+10/级", costBase: 1e3, mul: 1 },
altar: { name: "祭坛", des: "掉率+5/级 爆伤+5/级", costBase: 2e3, mul: 2 }
}
};
...
var lv = cc.getCampLv();
return cc.campCfg.lvCost[Math.min(9, lv)];
};
cc.upgradeCamp = function() {
if (!cc.playerData) return 1;
var lv = cc.playerData.campLv || 1;
...
var lv = cc.getFacLv(k);
return Math.floor(c.costBase * (lv + 1) * c.mul);
};
...
// 营地设施全局加成(合并到getplayerbsproperty)
cc.getCampProperty = function() {
var prop = [];
if (!cc.playerData || !cc.playerData.campFac) return prop;
var f = cc.playerData.campFac;
if (f.pet) { for (var i = 0; i < f.pet; i++) prop.push([ 2, 1 ], [ 3, 1 ], [ 4, 1 ], [ 5, 1 ], [ 6, 1 ], [ 1, 1 ]); }
if (f.forge) prop.push([ 21, f.forge * 10 ]);
if (f.pond) prop.push([ 8, f.pond * 200 ]);
if (f.train) prop.push([ 9, f.train * 20 ]);
