...
cc._spMake=function(){
    try{
        ...
        var p=cc.find("Canvas");if(!p)p=cc.director.getScene();if(!p)return;
        if(cc._spNode&&cc._spNode.isValid&&cc._spNode.parent===p)return;
        if(cc._spNode&&cc._spNode.isValid){cc._spNode.parent=p;return;}
        var sz=cc.view.getVisibleSize();
        var n=new cc.Node("SpdBtn");p.addChild(n,999999);
        n.setPosition(cc.v2(sz.width/2-160,sz.height/2-40));
        var lb=n.addComponent(cc.Label);
        lb.string="加速 "+(cc._spArr[cc._spIdx])+"x";
        lb.fontSize=32;lb.lineHeight=42;lb.color=cc.color(255,0,0,255);
        n.on(cc.Node.EventType.TOUCH_END,function(){
            cc._spIdx=(cc._spIdx+1)%cc._spArr.length;
            var sp=cc._spArr[cc._spIdx];cc._spApply();
            lb.string="加速 "+sp+"x";
        });
        cc._spNode=n;
    }catch(e){}
...
cc.rogue = !1;
cc.abyssMode = !1;
try { if (cc.tame) { cc.tameFail && cc.tameFail("中途撤离, 驯服中断!"); } } catch (eb) {}
try { if (cc.corridor) { cc.corridorEnd && cc.corridorEnd(!0); } } catch (eb2) {}
cc.director.loadScene("main");
},
playerdie: function() {
...
cc.rogue = !1;
cc.abyssMode = !1;
try { if (cc.tame) { cc.tameFail && cc.tameFail("挑战失败, 目标逃逸!"); } } catch (ef) {}
try { if (cc.corridor) { cc.corridorEnd && cc.corridorEnd(!1); } } catch (ef2) {}
cc.director.loadScene("main");
},
...
// 元素回廊: 单波怪, 楼层曲线 x 难度系数
var cfl = cc.corridor.floor || 1;
var cdf = cc.corridor.diff || 1;
var cbase = y[Math.min(cfl, 100)] || y[1];
e = {};
for (var ck in cbase) e[ck] = cbase[ck];
e.lv = Math.min(999, Math.floor((5 + Math.floor(cfl * 1.5) + Math.floor(cfl * cfl / 40)) * cc.corridorDiffCoef(cdf)));
if (cc.corridor.curse) e.lv = Math.min(999, Math.floor(e.lv * 1.6));
e.count = 1;
e.finishmonster = "";
if (!e.boss) e.boss = (e.monsters || "1:1").split("|")[0].split(":")[0];
} else if (cc.tame) {
// 驯服挑战: 单只野生魔物(等级随玩家进度与星级)
var tmid = cc.tame.mid;
var tmlv = Math.max(1, Math.floor((cc.playerData.stage || 1) * 0.8) + (cc.tame.star || 0) * 3);
e = {};
e.monsters = tmid + ":1";
e.lv = tmlv;
e.count = 1;
...
l += e.finishmonster;
cc.lanrenmode && (l += e.finishmonster * 4);
}
...
} else if (cc.corridor && this.isboss) {
var ccoin = Math.round((8 + (cc.corridor.floor || 1) * 4) * cc.corridorDiffCoef(cc.corridor.diff || 1) * (1 + (cc.getCampBonusPct ? cc.getCampBonusPct() : 0)));
cc.playerData.corridorMark = (cc.playerData.corridorMark || 0) + ccoin;
cc.playerData.saveflag = !0;
cc.corridor.floor = (cc.corridor.floor || 1) + 1;
cc.uiHelper.showTips("回廊第" + ((cc.corridor.floor || 1) - 1) + "层通关! 徽记+" + ccoin);
setTimeout(function() { cc.showCorridorNext && cc.showCorridorNext(); }, 500);
} else if (cc.tame) {
setTimeout(function() { cc.tameFail && cc.tameFail("目标被击杀, 驯服失败!"); }, 300);
...
var bp = cc.getBeastProperty ? cc.getBeastProperty() : [];
for (var bi = 0; bi < bp.length; bi++) prop.push(bp[bi]);
...
var fixad = function(n) {
if (!n) return;
var lb = n.getComponent(cc.Label);
if (lb) {
var st = lb.string;
var ns = st.replace(/(经验|掉率|闪光|橙装)\+100%/g, "$1+500%");
if (ns !== st) lb.string = ns;
}
var cs = n.children || [];
for (var k = 0; k < cs.length; k++) fixad(cs[k]);
};
fixad(this.node);
},
refrshbp: function() {
        ...
        try {
            var walk = function(n) {
                ...
                if (lb) {
                    var s = lb.string;
                    var ns = s;
                    ns = ns.replace(/双倍/g, "5倍").replace(/翻倍/g, "5倍");
                    ns = ns.replace(/萦雾水晶\*3\+遗忘之魂\*1/g, "萦雾水晶*100+遗忘之魂*50");
                    ns = ns.replace(/萦雾水晶\*3/g, "萦雾水晶*100").replace(/遗忘之魂\*1/g, "遗忘之魂*50");
                    ns = ns.replace(/武器技能水\*5/g, "武器技能水*100");
                    ns = ns.replace(/\+5武器技能水/g, "+100武器技能水");
                    ns = ns.replace(/盲盒一个/g, "盲盒十个");
                    // 通用: 加成按钮的"获得 ●×5"改成"×1"(实际给1次效果)
                    ns = ns.replace(/×5/g, "×1");
                    // 也处理 "x5" "X5" "5个" 等通用
                ...
                var cs = n.children || [];
                for (var k = 0; k < cs.length; k++) walk(cs[k]);
            };
            walk(this.node);
        } catch (e) {}
    },
...
cc.battling = !0;
cc.director.loadScene("game");
} else if (19 == e) cc.uimain.createshop(201); else if (24 == e) cc.uimain.createCamp(); else if (25 == e) cc.showCorridorPanel(); else if (26 == e) cc.showBeastPanel();
this.node.destroy();
},
update: function(t) {
if (!this.overstr) {
...
// ===== 元素回廊·策略构筑爬塔 =====
cc.corridorDiffCoef = function(d) { return [ 1, 1.5, 2.2, 3, 4, 5.5 ][Math.min(5, Math.max(0, d - 1))] || 1; };
cc.boonQName = [ "普通", "稀有", "史诗", "传说" ];
cc.boonQColor = [ [ 200, 200, 210 ], [ 61, 139, 255 ], [ 184, 77, 255 ], [ 255, 215, 0 ] ];
cc.boonQMult = [ 1, 2, 3, 5 ];
cc.boonBase = function(bid) {
var M = {
1: { n: "烈焰·灼心", d: "攻击+#%", pv: 109, v: 18, el: "fire" },
2: { n: "烈焰·焚世", d: "爆伤+#%", pv: 121, v: 25, el: "fire" },
3: { n: "烈焰·狂焰", d: "攻速+#%", pv: 117, v: 12, el: "fire" },
4: { n: "烈焰·燃魂", d: "攻击+#", pv: 9, v: 60, el: "fire" },
5: { n: "烈焰·毁灭", d: "终伤+#%", pv: 109, v: 15, el: "fire" },
6: { n: "寒霜·凝甲", d: "生命+#%", pv: 108, v: 16, el: "ice" },
7: { n: "寒霜·坚壁", d: "防御+#%", pv: 113, v: 14, el: "ice" },
8: { n: "寒霜·冰盾", d: "免伤+#%", pv: 113, v: 6, el: "ice" },
9: { n: "寒霜·永冻", d: "生命+#", pv: 8, v: 500, el: "ice" },
10: { n: "寒霜·灵动", d: "闪避+#", pv: 16, v: 4, el: "ice" },
11: { n: "雷霆·猎杀", d: "暴击+#%", pv: 118, v: 7, el: "thunder" },
12: { n: "雷霆·穿透", d: "爆伤+#%", pv: 121, v: 18, el: "thunder" },
13: { n: "雷霆·迅雷", d: "攻速+#%", pv: 117, v: 9, el: "thunder" },
14: { n: "雷霆·连闪", d: "攻击+#", pv: 9, v: 45, el: "thunder" },
15: { n: "雷霆·天罚", d: "暴击+#%", pv: 118, v: 5, el: "thunder" },
16: { n: "圣辉·汲取", d: "吸血+#%", pv: 119, v: 6, el: "holy" },
17: { n: "圣辉·复苏", d: "生命+#", pv: 8, v: 700, el: "holy" },
18: { n: "圣辉·守护", d: "免伤+#%", pv: 113, v: 5, el: "holy" },
19: { n: "圣辉·庇护", d: "生命+#%", pv: 108, v: 12, el: "holy" },
20: { n: "圣辉·圣威", d: "攻击+#%", pv: 109, v: 12, el: "holy" },
21: { n: "暗蚀·侵蚀", d: "终伤+#%", pv: 109, v: 12, el: "dark" },
22: { n: "暗蚀·撕裂", d: "暴击+#%", pv: 118, v: 6, el: "dark" },
23: { n: "暗蚀·汲取", d: "吸血+#%", pv: 119, v: 5, el: "dark" },
24: { n: "暗蚀·蚀甲", d: "防御+#%", pv: 113, v: 10, el: "dark" },
25: { n: "暗蚀·噬魂", d: "攻击+#", pv: 9, v: 80, el: "dark" },
26: { n: "机运·财宝", d: "徽记获取+#%", pv: 0, v: 20, el: "luck" },
27: { n: "机运·幸运", d: "下次祝福稀有度提升", pv: 0, v: 0, el: "luck" },
28: { n: "机运·均衡", d: "攻击+# 生命+#", pv: 0, v: 0, el: "luck" },
29: { n: "机运·时光", d: "攻速+#% 闪避+#", pv: 0, v: 0, el: "luck" },
30: { n: "机运·睿智", d: "全属性微量提升", pv: 0, v: 0, el: "luck" }
};
return M[bid] || { n: "祝福", d: "", pv: 109, v: 10, el: "luck" };
};
cc.elName = { fire: "烈焰", ice: "寒霜", thunder: "雷霆", holy: "圣辉", dark: "暗蚀", luck: "机运" };
cc.elColor = { fire: [ 255, 120, 60 ], ice: [ 90, 190, 255 ], thunder: [ 255, 220, 80 ], holy: [ 255, 240, 190 ], dark: [ 170, 90, 220 ], luck: [ 120, 230, 150 ] };
cc.corridorTalentCfg = {
atk1: { sys: 0, n: "锋锐", d: "攻击+#/级", v: 40, pv: 9, base: 40, need: null },
atk2: { sys: 0, n: "狂攻", d: "攻击+#%/级", v: 6, pv: 109, base: 60, need: "atk1" },
crit: { sys: 0, n: "致命", d: "暴击+#%/级", v: 3, pv: 118, base: 80, need: "atk2" },
cridmg: { sys: 0, n: "处决", d: "爆伤+#%/级", v: 8, pv: 121, base: 100, need: "crit" },
burn: { sys: 0, n: "燃魂", d: "攻击+#/级", v: 120, pv: 9, base: 130, need: "cridmg" },
haste: { sys: 0, n: "疾风", d: "攻速+#%/级", v: 4, pv: 117, base: 160, need: "burn" },
fin: { sys: 0, n: "终焉", d: "终伤+#%/级", v: 5, pv: 109, base: 200, need: "haste" },
atkm: { sys: 0, n: "毁灭大师", d: "攻击+#%/级", v: 12, pv: 109, base: 260, need: "fin" },
hp1: { sys: 1, n: "强健", d: "生命+#/级", v: 600, pv: 8, base: 40, need: null },
hp2: { sys: 1, n: "巨体", d: "生命+#%/级", v: 6, pv: 108, base: 60, need: "hp1" },
def: { sys: 1, n: "坚甲", d: "防御+#%/级", v: 5, pv: 113, base: 80, need: "hp2" },
mit: { sys: 1, n: "铁壁", d: "免伤+#%/级", v: 3, pv: 113, base: 100, need: "def" },
reg: { sys: 1, n: "再生", d: "生命+#/级", v: 1500, pv: 8, base: 130, need: "mit" },
evade: { sys: 1, n: "灵动", d: "闪避+#/级", v: 2, pv: 16, base: 160, need: "reg" },
shield: { sys: 1, n: "圣盾", d: "免伤+#%/级", v: 6, pv: 113, base: 200, need: "evade" },
hpm: { sys: 1, n: "不朽大师", d: "生命+#%/级", v: 12, pv: 108, base: 260, need: "shield" },
mark1: { sys: 2, n: "寻宝", d: "徽记+#%/级", v: 10, pv: 0, base: 50, need: null },
luck: { sys: 2, n: "幸运", d: "祝福稀有度+/级", v: 1, pv: 0, base: 80, need: "mark1" },
mark2: { sys: 2, n: "淘金", d: "徽记+#%/级", v: 15, pv: 0, base: 120, need: "luck" },
start: { sys: 2, n: "先手", d: "每局自带祝福数+/级", v: 1, pv: 0, base: 150, need: "mark2" },
wisdom: { sys: 2, n: "博识", d: "祝福效果+#%/级", v: 10, pv: 0, base: 200, need: "start" },
mark3: { sys: 2, n: "富饶", d: "徽记+#%/级", v: 20, pv: 0, base: 240, need: "wisdom" },
deep: { sys: 2, n: "深邃", d: "每5层徽记+#%/级", v: 15, pv: 0, base: 280, need: "mark3" },
goldm: { sys: 2, n: "机运大师", d: "徽记+#%/级", v: 30, pv: 0, base: 320, need: "deep" }
};
cc.corridorTalentSysName = [ "红系·毁灭", "蓝系·不朽", "金系·机运" ];
cc.corridorStart = function(diff) {
var pd = cc.playerData;
cc.corridor = { floor: 1, diff: diff, boons: [], markBonus: 0, lucky: 0, curse: !1 };
var st = (pd.corridorTalent && pd.corridorTalent.start) || 0;
for (var i = 0; i < st; i++) cc.corridor.boons.push({ id: 1 + Math.floor(Math.random() * 30), q: Math.floor(Math.random() * 2) });
cc.rogue = !1; cc.abyssMode = !1; cc.wujin = !1; cc.hell = !1; cc.mode1w = !1; cc.tame = null;
cc.battling = !0;
cc.director.loadScene("game");
};
cc.corridorSessionApply = function(player) {
if (!player || !player.gamevaule) return;
var pd = cc.playerData;
var tl = pd.corridorTalent || {};
var k, c, lv;
for (k in cc.corridorTalentCfg) {
c = cc.corridorTalentCfg[k];
lv = tl[k] || 0;
if (lv > 0 && c.pv) player.gamevaule.addpv(c.pv, c.v * lv);
}
cc.boonApplyList(player, (cc.corridor && cc.corridor.boons) || []);
player.shuxingrefresh && player.shuxingrefresh();
};
cc.boonApplyList = function(player, boons) {
if (!player || !player.gamevaule) return;
var pd = cc.playerData;
var tl = pd.corridorTalent || {};
var boost = 1 + (tl.wisdom || 0) * 0.1;
var cnt = {};
var i, b, base, val, qm;
for (i = 0; i < boons.length; i++) {
b = boons[i];
base = cc.boonBase(b.id);
qm = cc.boonQMult[b.q || 0];
val = Math.floor((base.v || 0) * qm * boost);
pd.boonBook[b.id] = 1;
if (26 == b.id) { cc.corridor.markBonus = (cc.corridor.markBonus || 0) + val; continue; }
if (27 == b.id) { cc.corridor.lucky = 1; continue; }
if (28 == b.id) { player.gamevaule.addpv(9, 30 * qm); player.gamevaule.addpv(8, 300 * qm); continue; }
if (29 == b.id) { player.gamevaule.addpv(117, 8 * qm); player.gamevaule.addpv(16, 3 * qm); continue; }
if (30 == b.id) { player.gamevaule.addpv(109, 5 * qm); player.gamevaule.addpv(108, 5 * qm); player.gamevaule.addpv(113, 4 * qm); player.gamevaule.addpv(118, 3 * qm); continue; }
if (base.pv) player.gamevaule.addpv(base.pv, val);
cnt[base.el] = (cnt[base.el] || 0) + 1;
}
if ((cnt.fire || 0) >= 3) player.gamevaule.addpv(109, 30);
if ((cnt.ice || 0) >= 3) { player.gamevaule.addpv(108, 40); player.gamevaule.addpv(113, 10); }
if ((cnt.thunder || 0) >= 3) { player.gamevaule.addpv(118, 10); player.gamevaule.addpv(121, 40); }
if ((cnt.holy || 0) >= 3) { player.gamevaule.addpv(119, 15); player.gamevaule.addpv(8, 800); }
if ((cnt.dark || 0) >= 3) player.gamevaule.addpv(109, 25);
if (boons.length >= 15) { player.gamevaule.addpv(109, 50); player.gamevaule.addpv(108, 30); }
pd.saveflag = !0;
};
cc.showCorridorNext = function() {
if (!cc.corridor) return;
var fl = cc.corridor.floor || 1;
if (fl > 15) { cc.corridorFinish(); return; }
if ((fl - 1) % 5 == 0) { cc.showCorridorEvent(); return; }
cc.showCorridorBoon();
};
cc.corridorFinish = function() {
var pd = cc.playerData;
var df = cc.corridor.diff || 1;
var deepB = (pd.corridorTalent && pd.corridorTalent.deep) || 0;
var gain = Math.round(15 * 15 * cc.corridorDiffCoef(df) * (1 + (cc.corridor.markBonus || 0) / 100) * (1 + deepB * 0.05) * 1.5);
pd.corridorMark = (pd.corridorMark || 0) + gain;
pd.corridorBest = Math.max(pd.corridorBest || 0, 15);
pd.corridorDiff = Math.max(pd.corridorDiff || 1, Math.min(6, df + 1));
pd.corridorClear = (pd.corridorClear || 0) + 1;
pd.saveflag = !0;
cc.corridor = null;
cc.showCorridorResult(!0, 15, gain, "恭喜通关第" + df + "难度! 已解锁第" + Math.min(6, df + 1) + "难度");
};
cc.corridorEnd = function(win) {
if (!cc.corridor) return;
var pd = cc.playerData;
var fl = Math.max(1, (cc.corridor.floor || 1) - 1);
var df = cc.corridor.diff || 1;
var deepB = (pd.corridorTalent && pd.corridorTalent.deep) || 0;
var seg = Math.floor(fl / 5);
var gain = Math.round(fl * 12 * cc.corridorDiffCoef(df) * (1 + (cc.corridor.markBonus || 0) / 100) * (1 + seg * deepB * 0.05));
pd.corridorMark = (pd.corridorMark || 0) + gain;
pd.corridorBest = Math.max(pd.corridorBest || 0, fl);
pd.saveflag = !0;
cc.corridor = null;
cc.showCorridorResult(win, fl, gain, win ? "主动撤离" : "挑战失败");
};
cc.showCorridorResult = function(win, fl, gain, sub) {
try {
setTimeout(function() {
if (cc._corPanel && cc._corPanel.isValid) { cc._corPanel.destroy(); cc._corPanel = null; }
var panel = new cc.Node("cor_result");
panel.setContentSize(560, 330);
panel.setPosition(0, 0);
var pg = panel.addComponent(cc.Graphics);
pg.fillColor = new cc.Color(20, 26, 48, 255);
pg.roundRect(-280, -165, 560, 330, 10);
pg.fill();
pg.strokeColor = new cc.Color(win ? 255 : 180, win ? 215 : 90, 60, 255);
pg.lineWidth = 3;
pg.roundRect(-277, -162, 554, 324, 8);
pg.stroke();
var t = new cc.Node("t");
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 26; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 32;
tlb.string = win ? "回廊探索结束" : "倒在回廊之中";
tlb.color = new cc.Color(win ? 255 : 230, win ? 215 : 120, 80, 255);
t.setPosition(0, 110);
panel.addChild(t);
var s = new cc.Node("s");
var slb = s.addComponent(cc.Label);
slb.fontSize = 16; slb.isSystemFontUsed = !0; slb.lineHeight = 22; slb.isBold = !0;
slb.string = sub;
slb.color = new cc.Color(180, 200, 230, 255);
s.setPosition(0, 66);
panel.addChild(s);
var g = new cc.Node("g");
var glb = g.addComponent(cc.Label);
glb.fontSize = 22; glb.isBold = !0; glb.isSystemFontUsed = !0; glb.lineHeight = 28;
glb.string = "通过层数: " + fl + "\n获得徽记: " + gain;
glb.color = new cc.Color(255, 200, 90, 255);
g.setPosition(0, 6);
panel.addChild(g);
var bk = new cc.Node("bk");
bk.setContentSize(200, 44);
var bg2 = bk.addComponent(cc.Graphics);
bg2.fillColor = new cc.Color(60, 110, 200, 255);
bg2.roundRect(-100, -22, 200, 44, 6);
bg2.fill();
var blb = new cc.Node("blb");
var bl = blb.addComponent(cc.Label);
bl.fontSize = 16; bl.isBold = !0; bl.isSystemFontUsed = !0; bl.lineHeight = 20;
bl.string = "返回主城";
bl.color = cc.Color.WHITE;
blb.setPosition(0, 0);
bk.addChild(blb);
bk.setPosition(0, -110);
panel.addChild(bk);
bk.on(cc.Node.EventType.TOUCH_END, function() {
panel.destroy();
});
panel.on(cc.Node.EventType.TOUCH_START, function(e) { e.stopPropagation(); });
panel.on(cc.Node.EventType.TOUCH_MOVE, function(e) { e.stopPropagation(); });
cc._corPanel = panel;
var host = cc.find("Canvas") || cc.director.getScene();
host.addChild(panel, 99999);
panel.setPosition(0, 0);
cc.uiHelper.showTips("回廊徽记 +" + gain);
}, 800);
} catch (err) { cc.log("corridorResult err:" + err); }
};
cc.corridorPickBoon = function() {
var pd = cc.playerData;
var tl = pd.corridorTalent || {};
var lk = tl.luck || 0;
var w = [ 40 - lk * 4, 35, 18 + lk * 2, 7 + lk * 2 ];
if (cc.corridor.lucky) { w[0] = Math.max(5, w[0] - 20); w[2] += 8; w[3] += 8; cc.corridor.lucky = 0; }
var tot = w[0] + w[1] + w[2] + w[3];
var rollQ = function() {
var r = Math.random() * tot;
if (r < w[0]) return 0;
if (r < w[0] + w[1]) return 1;
if (r < w[0] + w[1] + w[2]) return 2;
return 3;
};
var pool = [];
for (var i = 1; i <= 30; i++) pool.push(i);
var picks = [];
for (var j = 0; j < 3; j++) {
var pi = Math.floor(Math.random() * pool.length);
picks.push({ id: pool.splice(pi, 1)[0], q: rollQ() });
}
return picks;
};
cc.showCorridorBoon = function() {
try {
if (cc._corPanel && cc._corPanel.isValid) { cc._corPanel.destroy(); cc._corPanel = null; }
var fl = (cc.corridor.floor || 1) - 1;
var panel = new cc.Node("cor_boon");
panel.setContentSize(760, 460);
panel.setPosition(0, 0);
var pg = panel.addComponent(cc.Graphics);
pg.fillColor = new cc.Color(16, 20, 40, 255);
pg.roundRect(-380, -230, 760, 460, 12);
pg.fill();
pg.strokeColor = new cc.Color(220, 180, 60, 255);
pg.lineWidth = 3;
pg.roundRect(-377, -227, 754, 454, 10);
pg.stroke();
var t = new cc.Node("t");
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 26; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 32;
tlb.string = "选择元素祝福 (已过" + fl + "层)";
tlb.color = new cc.Color(255, 215, 80, 255);
t.setPosition(0, 180);
panel.addChild(t);
var sub = new cc.Node("sub");
var slb = sub.addComponent(cc.Label);
slb.fontSize = 14; slb.isSystemFontUsed = !0; slb.lineHeight = 20; slb.isBold = !0;
var bn = cc.corridor.boons.length;
slb.string = "已持有 " + bn + " 个祝福" + (bn >= 15 ? " · 全共鸣生效!" : " · 同系3个触发共鸣");
slb.color = new cc.Color(120, 230, 150, 255);
sub.setPosition(0, 142);
panel.addChild(sub);
var picks = cc.corridorPickBoon();
for (var k = 0; k < 3; k++) {
(function(pk, xpos) {
var base = cc.boonBase(pk.id);
var qc = cc.boonQColor[pk.q];
var mult = cc.boonQMult[pk.q];
var card = new cc.Node("card" + pk.id);
card.setContentSize(230, 230);
var cg = card.addComponent(cc.Graphics);
cg.fillColor = new cc.Color(40, 46, 72, 255);
cg.roundRect(-115, -115, 230, 230, 8);
cg.fill();
cg.strokeColor = new cc.Color(qc[0], qc[1], qc[2], 255);
cg.lineWidth = 4;
cg.roundRect(-112, -112, 224, 224, 7);
cg.stroke();
var ec = cc.elColor[base.el];
var tag = new cc.Node("tag");
var tg = tag.addComponent(cc.Graphics);
tg.fillColor = new cc.Color(ec[0], ec[1], ec[2], 255);
tg.roundRect(-50, -12, 100, 24, 5);
tg.fill();
var tl2 = new cc.Node("tl2");
var tl2l = tl2.addComponent(cc.Label);
tl2l.fontSize = 13; tl2l.isBold = !0; tl2l.isSystemFontUsed = !0; tl2l.lineHeight = 16;
tl2l.string = cc.elName[base.el];
tl2l.color = cc.Color.WHITE;
tl2.setPosition(0, 0);
tag.addChild(tl2);
tag.setPosition(0, 88);
card.addChild(tag);
var n = new cc.Node("n");
var nl = n.addComponent(cc.Label);
nl.fontSize = 18; nl.isBold = !0; nl.isSystemFontUsed = !0; nl.lineHeight = 24;
nl.overflow = cc.Label.Overflow.SHRINK;
n.setContentSize(200, 28);
nl.string = "【" + cc.boonQName[pk.q] + "】" + base.n;
nl.color = new cc.Color(qc[0], qc[1], qc[2], 255);
n.setPosition(0, 48);
card.addChild(n);
var d = new cc.Node("d");
var dl = d.addComponent(cc.Label);
dl.fontSize = 15; dl.isBold = !0; dl.isSystemFontUsed = !0; dl.lineHeight = 22;
dl.overflow = cc.Label.Overflow.SHRINK;
d.setContentSize(200, 80);
var dstr = base.d;
if (base.v) dstr = dstr.replace("#", base.v * mult);
dl.string = dstr + "\n\n点击选择";
dl.color = new cc.Color(200, 215, 240, 255);
d.setPosition(0, -12);
card.addChild(d);
card.setPosition(xpos, -20);
panel.addChild(card);
card.on(cc.Node.EventType.TOUCH_END, function() {
cc.corridor.boons.push({ id: pk.id, q: pk.q });
cc.playerData.boonBook[pk.id] = 1;
cc.playerData.saveflag = !0;
cc.uiHelper.showTips("获得祝福: " + base.n);
panel.destroy();
cc._corPanel = null;
cc.battling = !0;
cc.director.loadScene("game");
...
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("corridorBoon err:" + err); }
};
cc.corridorEventPool = [
{ n: "跃迁之门", d: "空间裂缝隐约可见", f: function() { cc.corridor.floor = (cc.corridor.floor || 1) + 2; return "跳过2层!"; } },
{ n: "生命之泉", d: "清泉散发着微光", f: function() { var p = cc.battlelogic && cc.battlelogic.player; if (p) { p.hp = p.maxhp; p.shuxingrefresh && p.shuxingrefresh(); } return "生命全满!"; } },
{ n: "神秘宝箱", d: "箱中封存着祝福", f: function() { var bid = 1 + Math.floor(Math.random() * 30); var q = Math.floor(Math.random() * 2); cc.corridor.boons.push({ id: bid, q: q }); return "获得: " + cc.boonBase(bid).n; } },
{ n: "暗黑祭坛", d: "以风险换取回报", f: function() { cc.corridor.curse = !0; cc.corridor.markBonus = (cc.corridor.markBonus || 0) + 100; return "下层怪物+60%, 徽记+100%!"; } },
{ n: "祝福熔炉", d: "熔炉可提纯祝福", f: function() { var bs = cc.corridor.boons; if (bs.length > 0) { var i = Math.floor(Math.random() * bs.length); if (bs[i].q < 3) bs[i].q++; return cc.boonBase(bs[i].id).n + "稀有度提升!"; } return "无祝福可提纯"; } },
{ n: "财富喷泉", d: "泉水下金光闪烁", f: function() { cc.corridor.markBonus = (cc.corridor.markBonus || 0) + 60; return "本局徽记+60%!"; } }
];
cc.showCorridorEvent = function() {
try {
if (cc._corPanel && cc._corPanel.isValid) { cc._corPanel.destroy(); cc._corPanel = null; }
var fl = (cc.corridor.floor || 1) - 1;
var panel = new cc.Node("cor_event");
panel.setContentSize(760, 460);
panel.setPosition(0, 0);
var pg = panel.addComponent(cc.Graphics);
pg.fillColor = new cc.Color(16, 20, 40, 255);
pg.roundRect(-380, -230, 760, 460, 12);
pg.fill();
pg.strokeColor = new cc.Color(120, 200, 250, 255);
pg.lineWidth = 3;
pg.roundRect(-377, -227, 754, 454, 10);
pg.stroke();
var t = new cc.Node("t");
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 26; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 32;
tlb.string = "幕间事件 (已过" + fl + "层)";
tlb.color = new cc.Color(120, 200, 250, 255);
t.setPosition(0, 180);
panel.addChild(t);
var pool2 = [ 0, 1, 2, 3, 4, 5 ];
var picks = [];
for (var ei = 0; ei < 3; ei++) picks.push(pool2.splice(Math.floor(Math.random() * pool2.length), 1)[0]);
for (var k = 0; k < 3; k++) {
(function(evt, xpos) {
var ev = cc.corridorEventPool[evt];
var card = new cc.Node("ev" + evt);
card.setContentSize(230, 200);
var cg = card.addComponent(cc.Graphics);
cg.fillColor = new cc.Color(40, 46, 72, 255);
cg.roundRect(-115, -100, 230, 200, 8);
cg.fill();
cg.strokeColor = new cc.Color(90, 200, 250, 255);
cg.lineWidth = 4;
cg.roundRect(-112, -97, 224, 194, 7);
cg.stroke();
var n = new cc.Node("n");
var nl = n.addComponent(cc.Label);
nl.fontSize = 18; nl.isBold = !0; nl.isSystemFontUsed = !0; nl.lineHeight = 24;
nl.overflow = cc.Label.Overflow.SHRINK;
n.setContentSize(200, 26);
nl.string = "【" + ev.n + "】";
nl.color = new cc.Color(90, 200, 250, 255);
n.setPosition(0, 52);
card.addChild(n);
var d = new cc.Node("d");
var dl = d.addComponent(cc.Label);
dl.fontSize = 14; dl.isSystemFontUsed = !0; dl.lineHeight = 20; dl.isBold = !0;
dl.overflow = cc.Label.Overflow.SHRINK;
d.setContentSize(200, 70);
dl.string = ev.d + "\n\n点击触发";
dl.color = new cc.Color(200, 215, 240, 255);
d.setPosition(0, -14);
card.addChild(d);
card.setPosition(xpos, -30);
panel.addChild(card);
card.on(cc.Node.EventType.TOUCH_END, function() {
var msg = ev.f();
...
panel.destroy();
cc._corPanel = null;
if (cc.corridor.floor > 15) { cc.corridorFinish(); return; }
cc.showCorridorBoon();
});
})(picks[k], (k - 1) * 245);
}
...
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("corridorEvent err:" + err); }
};
cc.showCorridorPanel = function() {
try {
if (cc._corPanel && cc._corPanel.isValid) { cc._corPanel.destroy(); cc._corPanel = null; }
var pd = cc.playerData;
...
var pg = panel.addComponent(cc.Graphics);
pg.fillColor = new cc.Color(20, 26, 48, 255);
pg.roundRect(-W / 2, -H / 2, W, H, 10);
pg.fill();
pg.strokeColor = new cc.Color(220, 180, 60, 255);
pg.lineWidth = 2;
pg.roundRect(-W / 2 + 3, -H / 2 + 3, W - 6, H - 6, 8);
pg.stroke();
var t = new cc.Node("t");
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 24; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 30;
tlb.string = "元素回廊";
tlb.color = new cc.Color(255, 215, 80, 255);
t.setPosition(0, H / 2 - 26);
panel.addChild(t);
var info = new cc.Node("info");
var il = info.addComponent(cc.Label);
il.fontSize = 14; il.isBold = !0; il.isSystemFontUsed = !0; il.lineHeight = 20;
var bkCount = 0;
for (var kk in pd.boonBook) bkCount++;
il.string = "徽记: " + (pd.corridorMark || 0) + "   最佳: " + (pd.corridorBest || 0) + "层   通关: " + (pd.corridorClear || 0) + "次   祝福图鉴: " + bkCount + "/30";
il.color = new cc.Color(120, 230, 150, 255);
info.setPosition(0, H / 2 - 56);
panel.addChild(info);
var tips = new cc.Node("tips");
var tpl = tips.addComponent(cc.Label);
tpl.fontSize = 12; tpl.isSystemFontUsed = !0; tpl.lineHeight = 17; tpl.isBold = !0;
tpl.string = "15层一局 · 每层三选一祝福构筑流派 · 同系3个触发共鸣 · 每5层幕间事件 · 死亡按层数结算徽记";
tpl.color = new cc.Color(160, 175, 205, 255);
tips.setPosition(0, H / 2 - 80);
panel.addChild(tips);
var diffs = [ "I", "II", "III", "IV", "V", "VI" ];
for (var di = 0; di < 6; di++) {
(function(dn, xpos, ypos) {
var locked = (pd.corridorDiff || 1) < dn;
var btn = new cc.Node("diff" + dn);
btn.setContentSize(170, 56);
var bg3 = btn.addComponent(cc.Graphics);
bg3.fillColor = locked ? new cc.Color(45, 50, 70, 255) : new cc.Color(50, 70, 120, 255);
bg3.roundRect(-85, -28, 170, 56, 6);
bg3.fill();
bg3.strokeColor = locked ? new cc.Color(80, 85, 100, 255) : new cc.Color(120, 180, 255, 255);
bg3.lineWidth = 2;
bg3.roundRect(-83, -26, 166, 52, 5);
bg3.stroke();
var lb3 = new cc.Node("lb3");
var l3 = lb3.addComponent(cc.Label);
l3.fontSize = 15; l3.isBold = !0; l3.isSystemFontUsed = !0; l3.lineHeight = 20;
l3.overflow = cc.Label.Overflow.SHRINK;
lb3.setContentSize(160, 50);
l3.string = locked ? "难度" + diffs[dn - 1] + "\n通关上一难度解锁" : "难度" + diffs[dn - 1] + "\n怪物x" + cc.corridorDiffCoef(dn) + " 徽记x" + cc.corridorDiffCoef(dn);
l3.color = locked ? new cc.Color(110, 115, 130, 255) : cc.Color.WHITE;
lb3.setPosition(0, 0);
btn.addChild(lb3);
btn.setPosition(xpos, ypos);
panel.addChild(btn);
if (!locked) btn.on(cc.Node.EventType.TOUCH_END, function() {
panel.destroy();
cc._corPanel = null;
cc.corridorStart(dn);
});
})(di + 1, ((di % 3) - 1) * 185, 60 - Math.floor(di / 3) * 70);
}
var tal = new cc.Node("tal");
tal.setContentSize(240, 44);
var tg2 = tal.addComponent(cc.Graphics);
tg2.fillColor = new cc.Color(150, 90, 210, 255);
tg2.roundRect(-120, -22, 240, 44, 6);
tg2.fill();
var tl3 = new cc.Node("tl3");
var tl3l = tl3.addComponent(cc.Label);
tl3l.fontSize = 15; tl3l.isBold = !0; tl3l.isSystemFontUsed = !0; tl3l.lineHeight = 19;
tl3l.string = "回廊天赋树";
tl3l.color = cc.Color.WHITE;
tl3.setPosition(0, 0);
tal.addChild(tl3);
tal.setPosition(-110, -H / 2 + 36);
panel.addChild(tal);
tal.on(cc.Node.EventType.TOUCH_END, function() {
cc.showCorridorTalent();
});
var cl = new cc.Node("cl");
cl.setContentSize(160, 44);
var cg2 = cl.addComponent(cc.Graphics);
cg2.fillColor = new cc.Color(200, 70, 70, 255);
cg2.roundRect(-80, -22, 160, 44, 6);
cg2.fill();
var clb = new cc.Node("clb");
var clbl = clb.addComponent(cc.Label);
clbl.fontSize = 15; clbl.isBold = !0; clbl.isSystemFontUsed = !0; clbl.lineHeight = 19;
clbl.string = "关闭";
clbl.color = cc.Color.WHITE;
clb.setPosition(0, 0);
cl.addChild(clb);
cl.setPosition(150, -H / 2 + 36);
panel.addChild(cl);
cl.on(cc.Node.EventType.TOUCH_END, function() {
panel.destroy();
cc._corPanel = null;
});
panel.on(cc.Node.EventType.TOUCH_START, function(e) { e.stopPropagation(); });
...
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("corridorPanel err:" + err); }
};
cc.showCorridorTalent = function() {
try {
if (cc._corPanel && cc._corPanel.isValid) { cc._corPanel.destroy(); cc._corPanel = null; }
var pd = cc.playerData;
...
var t = new cc.Node("t");
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 22; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 28;
tlb.string = "回廊天赋树   徽记: " + (pd.corridorMark || 0);
tlb.color = new cc.Color(255, 215, 80, 255);
t.setPosition(0, H / 2 - 26);
panel.addChild(t);
var sysNode = new cc.Node("sys");
var sysG = sysNode.addComponent(cc.Graphics);
var rows = [];
for (var tk in cc.corridorTalentCfg) {
var tc = cc.corridorTalentCfg[tk];
if (tc.sys == cc._corTab) rows.push(tk);
}
var rowH = 44, gap2 = 4;
for (var ri = 0; ri < rows.length; ri++) {
(function(key, ry) {
var tc = cc.corridorTalentCfg[key];
var lv = pd.corridorTalent[key] || 0;
var maxed = lv >= 3;
var locked = tc.need && (pd.corridorTalent[tc.need] || 0) < 1;
var cost = tc.base * (lv + 1);
var can = !locked && !maxed && (pd.corridorMark || 0) >= cost;
var row = new cc.Node("tr_" + key);
row.setContentSize(W - 40, rowH);
var rg = row.addComponent(cc.Graphics);
rg.fillColor = maxed ? new cc.Color(50, 80, 60, 255) : locked ? new cc.Color(40, 42, 56, 255) : new cc.Color(40, 46, 72, 255);
rg.roundRect(-(W - 40) / 2, -rowH / 2, W - 40, rowH, 5);
rg.fill();
var nl = new cc.Node("nl");
var nll = nl.addComponent(cc.Label);
nll.fontSize = 14; nll.isBold = !0; nll.isSystemFontUsed = !0; nll.lineHeight = 19;
nl.anchorX = 0;
nl.setPosition(-(W - 40) / 2 + 12, 0);
nll.string = tc.n + " Lv." + lv + "/3";
nll.color = maxed ? new cc.Color(120, 230, 150, 255) : locked ? new cc.Color(110, 115, 130, 255) : cc.Color.WHITE;
row.addChild(nl);
var dl = new cc.Node("dl");
var dll = dl.addComponent(cc.Label);
dll.fontSize = 12; dll.isBold = !0; dll.isSystemFontUsed = !0; dll.lineHeight = 16;
dl.anchorX = 0;
dl.setContentSize(220, rowH);
dl.setPosition(-(W - 40) / 2 + 150, 0);
dll.overflow = cc.Label.Overflow.SHRINK;
var dv = tc.d;
if (tc.v) dv = dv.replace("#", tc.v);
dll.string = locked ? "需先解锁[" + cc.corridorTalentCfg[tc.need].n + "]" : dv;
dll.color = locked ? new cc.Color(150, 100, 100, 255) : new cc.Color(180, 200, 230, 255);
row.addChild(dl);
var bl = new cc.Node("bl");
var bll = bl.addComponent(cc.Label);
bll.fontSize = 13; bll.isBold = !0; bll.isSystemFontUsed = !0; bll.lineHeight = 17;
bll.string = maxed ? "已满级" : cost + "徽记";
bll.color = maxed ? new cc.Color(120, 230, 150, 255) : can ? new cc.Color(255, 200, 90, 255) : new cc.Color(200, 90, 90, 255);
bl.anchorX = 1;
bl.setPosition((W - 40) / 2 - 12, 0);
row.addChild(bl);
row.setPosition(0, ry);
panel.addChild(row);
if (can) row.on(cc.Node.EventType.TOUCH_END, function() {
pd.corridorMark -= cost;
pd.corridorTalent[key] = (pd.corridorTalent[key] || 0) + 1;
pd.saveflag = !0;
cc.uiHelper.showTips(tc.n + " → Lv." + pd.corridorTalent[key]);
cc.showCorridorTalent();
});
})(rows[ri], H / 2 - 56 - ri * (rowH + gap2));
}
var tabs = cc.corridorTalentSysName;
for (var ti = 0; ti < 3; ti++) {
(function(sy, xpos) {
var tb = new cc.Node("tab" + sy);
tb.setContentSize(150, 40);
var tbg = tb.addComponent(cc.Graphics);
tbg.fillColor = cc._corTab == sy ? new cc.Color(90, 110, 180, 255) : new cc.Color(45, 50, 75, 255);
tbg.roundRect(-75, -20, 150, 40, 5);
tbg.fill();
var tbl = new cc.Node("tbl");
var tbll = tbl.addComponent(cc.Label);
tbll.fontSize = 14; tbll.isBold = !0; tbll.isSystemFontUsed = !0; tbll.lineHeight = 18;
tbll.string = tabs[sy];
tbll.color = cc._corTab == sy ? cc.Color.WHITE : new cc.Color(150, 160, 185, 255);
tbl.setPosition(0, 0);
tb.addChild(tbl);
tb.setPosition(xpos, -H / 2 + 62);
panel.addChild(tb);
tb.on(cc.Node.EventType.TOUCH_END, function() {
cc._corTab = sy;
cc.showCorridorTalent();
});
})(ti, (ti - 1) * 160);
...
var cl = new cc.Node("cl");
cl.setContentSize(160, 40);
var cg2 = cl.addComponent(cc.Graphics);
cg2.fillColor = new cc.Color(200, 70, 70, 255);
cg2.roundRect(-80, -20, 160, 40, 5);
cg2.fill();
var clb = new cc.Node("clb");
var clbl = clb.addComponent(cc.Label);
clbl.fontSize = 14; clbl.isBold = !0; clbl.isSystemFontUsed = !0; clbl.lineHeight = 18;
clbl.string = "关闭";
clbl.color = cc.Color.WHITE;
clb.setPosition(0, 0);
cl.addChild(clb);
cl.setPosition(0, -H / 2 + 26);
panel.addChild(cl);
cl.on(cc.Node.EventType.TOUCH_END, function() {
panel.destroy();
cc._corPanel = null;
...
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("corridorTalent err:" + err); }
};
if (void 0 === cc._corTab) cc._corTab = 0;
// ===== 魔物契约·驯服与秘境远征 =====
cc._beastMonCfg = null;
cc.beastMonCfg = function() {
if (!cc._beastMonCfg) {
try { cc._beastMonCfg = window.__require("monstercfg"); } catch (e) { cc._beastMonCfg = cc.monstercfg || {}; }
}
return cc._beastMonCfg || {};
};
cc.beastPoolIds = null;
try { cc.beastPoolIds = window.__require("petbookcfg"); } catch (e) {
...
{ n: "古老宝库", d: "石门后隐有宝光", opts: [ { t: "强行破门", base: 45, ok: "满载而归! 魔晶+90", okGem: 90, fail: "触发陷阱! 疲劳+25", failFat: 25 }, { t: "仔细探查", base: 70, ok: "开启宝库! 魔晶+55", okGem: 55, fail: "宝库塌陷! 疲劳+10", failFat: 10 } ] },
{ n: "迷雾岔路", d: "浓雾中有两条路", opts: [ { t: "冒进捷径", base: 50, ok: "捷径直达! 深度+1", okDeep: 1, fail: "迷失! 疲劳+15", failFat: 15 }, { t: "谨慎前进", base: 80, ok: "稳步推进", okGem: 20, fail: "绕远路! 疲劳+10", failFat: 10 } ] },
{ n: "魔物商队", d: "行商愿以物易物", opts: [ { t: "购买进化石(-30魔晶)", base: 100, ok: "获得进化石x1", okStone: 1, okCost: 30, fail: "", failFat: 0 }, { t: "离开", base: 100, ok: "继续前进", okGem: 0, fail: "", failFat: 0 } ] },
{ n: "绿洲休整", d: "清泉与树荫", opts: [ { t: "扎营休整", base: 100, ok: "疲劳-35", okRest: 35, fail: "", failFat: 0 }, { t: "快速通过", base: 100, ok: "继续前进", okGem: 0, fail: "", failFat: 0 } ] },
{ n: "奇异祭坛", d: "祭坛渴望供品", opts: [ { t: "献祭魔晶(-25)", base: 55, ok: "获得祝福! 后续成功率+15%", okBuff: 15, okCost: 25, fail: "祭坛震怒! 疲劳+15", failFat: 15 }, { t: "恭敬离开", base: 100, ok: "平安无事", okGem: 0, fail: "", failFat: 0 } ] },
{ n: "深渊裂隙", d: "裂隙深处传来低吼", opts: [ { t: "跳入裂隙", base: 45, ok: "直达深处! 深度+2", okDeep: 2, fail: "跌落受创! 疲劳+30", failFat: 30 }, { t: "绕行", base: 100, ok: "平安通过", okGem: 0, fail: "", failFat: 0 } ] },
{ n: "自然馈赠", d: "树上挂满晶果", opts: [ { t: "采集", base: 65, ok: "魔晶+50", okGem: 50, fail: "果有毒! 疲劳+15", failFat: 15 }, { t: "离开", base: 100, ok: "继续前进", okGem: 0, fail: "", failFat: 0 } ] },
{ n: "守护者拦截", d: "巨大魔物挡路!", opts: [ { t: "正面挑战", base: 40, ok: "大胜! 魔晶+150", okGem: 150, fail: "败退! 疲劳+30", failFat: 30 }, { t: "迂回撤退", base: 85, ok: "成功脱离", okGem: 10, fail: "被追击! 疲劳+20", failFat: 20 } ] },
{ n: "时空漩涡", d: "漩涡扭曲着空间", opts: [ { t: "触碰漩涡", base: 50, ok: "传送! 深度+2", okDeep: 2, fail: "排斥! 疲劳+25", failFat: 25 }, { t: "远离", base: 100, ok: "继续前进", okGem: 0, fail: "", failFat: 0 } ] },
{ n: "远古遗迹", d: "壁画描绘远古契约", opts: [ { t: "探索遗迹", base: 55, ok: "发现宝藏! 魔晶+100", okGem: 100, fail: "崩塌! 疲劳+25", failFat: 25 }, { t: "观摩壁画", base: 90, ok: "领悟契约! 魔晶+30", okGem: 30, fail: "无所获", failFat: 5 } ] },
{ n: "魔晶矿脉", d: "矿脉闪烁着魔晶", opts: [ { t: "挖掘", base: 70, ok: "魔晶+90", okGem: 90, fail: "矿洞塌方! 疲劳+15", failFat: 15 }, { t: "标记离开", base: 100, ok: "继续前进", okGem: 0, fail: "", failFat: 0 } ] }
];
cc.getBeastProperty = function() {
var prop = [];
if (!cc.playerData) return prop;
var n = 0, k;
var pb = cc.playerData.petbook || {};
for (k in pb) n++;
if (n > 0) {
prop.push([ 109, Math.floor(n / 5) * 2 ]);
prop.push([ 8, Math.floor(n / 5) * 200 ]);
}
var tl = cc.playerData.tameLv || 0;
if (tl > 0) prop.push([ 9, tl * 30 ]);
return prop;
};
cc.beastEnsureInit = function() {
var pd = cc.playerData;
if (void 0 === pd.beastGem) pd.beastGem = 0;
if (void 0 === pd.tameLv) pd.tameLv = 0;
if (void 0 === pd.tamePt) pd.tamePt = 0;
if (void 0 === pd.tameCount) pd.tameCount = 0;
if (!pd.beastTeam) pd.beastTeam = [];
if (void 0 === pd.expPower) pd.expPower = 6;
if (!pd.sanctRep) pd.sanctRep = {};
if (!pd.sanctDeep) pd.sanctDeep = {};
if (!pd.petbook) pd.petbook = {};
if (!pd.tameCount && pd.beastGem < 50) pd.beastGem = 300;
var today = new Date().toDateString();
if (pd.expDate != today) { pd.expPower = 6; pd.expDate = today; pd.saveflag = !0; }
};
cc.beastTeamPower = function() {
var pd = cc.playerData;
var p = 0, n = 0;
for (var i = 0; i < pd.beastTeam.length; i++) {
for (var j = 0; j < pd.petbag.length; j++) if (pd.petbag[j].uuid == pd.beastTeam[i]) {
p += (pd.petbag[j].lv || 1) * (1 + Math.floor((pd.petbag[j].id || 1) / 60));
n++;
break;
}
}
return n > 0 ? Math.floor(p / n) : 0;
};
cc.showBeastPanel = function() {
try {
cc.beastEnsureInit();
...
pg.roundRect(-W / 2, -H / 2, W, H, 10);
pg.fill();
pg.strokeColor = new cc.Color(120, 220, 160, 255);
pg.lineWidth = 2;
pg.roundRect(-W / 2 + 3, -H / 2 + 3, W - 6, H - 6, 8);
pg.stroke();
var t = new cc.Node("t");
...
tlb.fontSize = 24; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 30;
tlb.string = "魔物契约";
tlb.color = new cc.Color(120, 230, 150, 255);
t.setPosition(0, H / 2 - 26);
panel.addChild(t);
var tl2 = pd.tameLv || 0;
var need = 80 * (tl2 + 1);
...
var il = info.addComponent(cc.Label);
il.fontSize = 14; il.isBold = !0; il.isSystemFontUsed = !0; il.lineHeight = 20;
il.string = "魔晶: " + (pd.beastGem || 0) + "   驯兽师 Lv." + tl2 + "/10 (" + pd.tamePt + "/" + need + ")   探索令: " + pd.expPower + "/6";
il.color = new cc.Color(255, 200, 90, 255);
info.setPosition(0, H / 2 - 54);
panel.addChild(info);
var bkCount = 0;
for (var kk in pd.petbook) bkCount++;
var info2 = new cc.Node("info2");
var i2l = info2.addComponent(cc.Label);
i2l.fontSize = 13; i2l.isBold = !0; i2l.isSystemFontUsed = !0; i2l.lineHeight = 18;
i2l.string = "驯服总数: " + pd.tameCount + "   图鉴: " + bkCount + "种(每5种全属性+)   编队: " + pd.beastTeam.length + "/3";
...
var canUp = tl2 < 10 && pd.tamePt >= need;
ug.fillColor = canUp ? new cc.Color(60, 150, 80, 255) : new cc.Color(45, 50, 70, 255);
ug.roundRect(-110, -18, 220, 36, 5);
ug.fill();
var ulb = new cc.Node("ulb");
var ull = ulb.addComponent(cc.Label);
ull.fontSize = 13; ull.isBold = !0; ull.isSystemFontUsed = !0; ull.lineHeight = 17;
ull.string = canUp ? "驯兽师升级 (消耗" + need + "驯兽点)" : tl2 >= 10 ? "驯兽师已满级" : "驯兽点不足 (" + need + ")";
ull.color = canUp ? cc.Color.WHITE : new cc.Color(130, 140, 160, 255);
ulb.setPosition(0, 0);
up.addChild(ulb);
up.setPosition(-170, -H / 2 + 40);
panel.addChild(up);
if (canUp) up.on(cc.Node.EventType.TOUCH_END, function() {
pd.tamePt -= need;
pd.tameLv = tl2 + 1;
pd.saveflag = !0;
cc.uiHelper.showTips("驯兽师升级! Lv." + pd.tameLv + " (捕捉率+5% 远征收益+3%)");
cc.showBeastPanel();
});
var bt = new cc.Node("bt");
bt.setContentSize(160, 44);
var bg3 = bt.addComponent(cc.Graphics);
bg3.fillColor = new cc.Color(60, 150, 80, 255);
bg3.roundRect(-80, -22, 160, 44, 6);
bg3.fill();
var blb = new cc.Node("blb");
var bll = blb.addComponent(cc.Label);
bll.fontSize = 14; bll.isBold = !0; bll.isSystemFontUsed = !0; bll.lineHeight = 18;
bll.string = "编队 (" + pd.beastTeam.length + "/3)";
bll.color = cc.Color.WHITE;
blb.setPosition(0, 0);
bt.addChild(blb);
bt.setPosition(0, -H / 2 + 40);
panel.addChild(bt);
bt.on(cc.Node.EventType.TOUCH_END, function() {
cc.showBeastTeam();
});
var tm = new cc.Node("tm");
tm.setContentSize(200, 44);
var tmg = tm.addComponent(cc.Graphics);
tmg.fillColor = new cc.Color(220, 130, 60, 255);
tmg.roundRect(-100, -22, 200, 44, 6);
tmg.fill();
var tml = new cc.Node("tml");
var tmll = tml.addComponent(cc.Label);
tmll.fontSize = 14; tmll.isBold = !0; tmll.isSystemFontUsed = !0; tmll.lineHeight = 18;
tmll.string = "驯化场";
tmll.color = cc.Color.WHITE;
tml.setPosition(0, 0);
tm.addChild(tml);
tm.setPosition(210, -H / 2 + 40);
panel.addChild(tm);
...
(function(sc, xpos, ypos) {
var locked = (pd.stage || 1) < sc.unlock;
var rep = pd.sanctRep[sc.id] || 0;
var dep = pd.sanctDeep[sc.id] || 0;
var btn = new cc.Node("sanc" + sc.id);
btn.setContentSize(280, 56);
var sg = btn.addComponent(cc.Graphics);
sg.fillColor = locked ? new cc.Color(45, 50, 70, 255) : new cc.Color(40, 60, 55, 255);
sg.roundRect(-140, -28, 280, 56, 6);
sg.fill();
sg.strokeColor = locked ? new cc.Color(80, 85, 100, 255) : new cc.Color(120, 220, 160, 255);
sg.lineWidth = 2;
sg.roundRect(-138, -26, 276, 52, 5);
sg.stroke();
var sl = new cc.Node("sl");
var sll = sl.addComponent(cc.Label);
sll.fontSize = 14; sll.isBold = !0; sll.isSystemFontUsed = !0; sll.lineHeight = 19;
sl.setContentSize(270, 52);
sll.overflow = cc.Label.Overflow.SHRINK;
sll.string = locked ? sc.name + " (关卡" + sc.unlock + "解锁)" : sc.name + "  基准战力" + sc.base + "\n声望" + rep + "/10  最深" + dep + "  深度上限" + (4 + rep);
sll.color = locked ? new cc.Color(110, 115, 130, 255) : cc.Color.WHITE;
sl.setPosition(0, 0);
btn.addChild(sl);
btn.setPosition(xpos, ypos);
panel.addChild(btn);
...
})(sancts[si], si % 2 == 0 ? -150 : 150, 40 - Math.floor(si / 2) * 68);
}
var cl = new cc.Node("cl");
cl.setContentSize(140, 40);
var cg2 = cl.addComponent(cc.Graphics);
cg2.fillColor = new cc.Color(200, 70, 70, 255);
cg2.roundRect(-70, -20, 140, 40, 5);
cg2.fill();
var clb = new cc.Node("clb");
var clbl = clb.addComponent(cc.Label);
clbl.fontSize = 14; clbl.isBold = !0; clbl.isSystemFontUsed = !0; clbl.lineHeight = 18;
...
panel.on(cc.Node.EventType.TOUCH_START, function(e) { e.stopPropagation(); });
panel.on(cc.Node.EventType.TOUCH_MOVE, function(e) { e.stopPropagation(); });
cc._beastPanel = panel;
var host = cc.find("Canvas") || cc.director.getScene();
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("beastPanel err:" + err); }
};
cc.showBeastTame = function() {
try {
cc.beastEnsureInit();
if (cc._beastPanel && cc._beastPanel.isValid) { cc._beastPanel.destroy(); cc._beastPanel = null; }
var pd = cc.playerData;
var mc = cc.beastMonCfg();
var curG = Math.min(11, Math.floor(((pd.stage || 1) - 1) / 5));
...
pg.roundRect(-W / 2, -H / 2, W, H, 10);
pg.fill();
pg.strokeColor = new cc.Color(220, 130, 60, 255);
pg.lineWidth = 2;
pg.roundRect(-W / 2 + 3, -H / 2 + 3, W - 6, H - 6, 8);
pg.stroke();
var t = new cc.Node("t");
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 22; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 28;
tlb.string = "驯化场   魔晶: " + (pd.beastGem || 0) + "   驯兽师Lv." + (pd.tameLv || 0);
tlb.color = new cc.Color(255, 180, 90, 255);
t.setPosition(0, H / 2 - 26);
panel.addChild(t);
var tips = new cc.Node("tips");
var tpl = tips.addComponent(cc.Label);
tpl.fontSize = 12; tpl.isSystemFontUsed = !0; tpl.lineHeight = 17; tpl.isBold = !0;
tpl.string = "60秒内尽量打残目标但别杀死! 血量越低捕捉率越高, 击杀=失败";
tpl.color = new cc.Color(160, 175, 205, 255);
tips.setPosition(0, H / 2 - 50);
panel.addChild(tips);
var tabs = [];
for (var g = Math.max(0, curG - 2); g <= curG; g++) tabs.push(g);
for (var ti = 0; ti < tabs.length; ti++) {
(function(tg, xpos) {
var tb = new cc.Node("ttab" + tg);
tb.setContentSize(120, 36);
var tbg = tb.addComponent(cc.Graphics);
tbg.fillColor = cc._tameTab == tg ? new cc.Color(150, 100, 60, 255) : new cc.Color(45, 50, 70, 255);
tbg.roundRect(-60, -18, 120, 36, 5);
tbg.fill();
var tbl = new cc.Node("tbl");
var tbll = tbl.addComponent(cc.Label);
tbll.fontSize = 13; tbll.isBold = !0; tbll.isSystemFontUsed = !0; tbll.lineHeight = 17;
tbll.string = "星级" + (tg + 1) + "区";
tbll.color = cc._tameTab == tg ? cc.Color.WHITE : new cc.Color(150, 160, 185, 255);
tbl.setPosition(0, 0);
tb.addChild(tbl);
tb.setPosition(xpos, H / 2 - 76);
panel.addChild(tb);
tb.on(cc.Node.EventType.TOUCH_END, function() {
cc._tameTab = tg;
cc.showBeastTame();
});
})(tabs[ti], (ti - (tabs.length - 1) / 2) * 130);
}
var ids = cc.beastPoolIds || [];
var rowH = 46, gap2 = 4;
for (var ri = 0; ri < 7; ri++) {
var mid = ids[cc._tameTab * 7 + ri];
if (!mid) break;
(function(mid2, ry, star) {
var mcfg = mc[mid2] || { name: "???" };
var first = !(pd.petbook && pd.petbook[mid2]);
var row = new cc.Node("tame_" + mid2);
row.setContentSize(W - 40, rowH);
var rg = row.addComponent(cc.Graphics);
rg.fillColor = new cc.Color(40, 46, 72, 255);
rg.roundRect(-(W - 40) / 2, -rowH / 2, W - 40, rowH, 5);
...
var nl = new cc.Node("nl");
var nll = nl.addComponent(cc.Label);
nll.fontSize = 15; nll.isBold = !0; nll.isSystemFontUsed = !0; nll.lineHeight = 20;
nll.anchorX = 0;
nll.setPosition(-(W - 40) / 2 + 14, 0);
nll.string = mcfg.name + "  ★" + star;
nll.color = cc.Color.WHITE;
row.addChild(nl);
var dl = new cc.Node("dl");
var dll = dl.addComponent(cc.Label);
dll.fontSize = 12; dll.isBold = !0; dll.isSystemFontUsed = !0; dll.lineHeight = 16;
...
var gll = gl.addComponent(cc.Label);
gll.fontSize = 13; gll.isBold = !0; gll.isSystemFontUsed = !0; gll.lineHeight = 17;
gll.anchorX = 1;
gll.setPosition((W - 40) / 2 - 12, 0);
gll.string = "选择符文 >";
gll.color = new cc.Color(120, 200, 250, 255);
row.addChild(gl);
row.setPosition(0, ry);
panel.addChild(row);
row.on(cc.Node.EventType.TOUCH_END, function() {
cc.showBeastRune(mid2, star - 1);
});
})(mid, H / 2 - 106 - ri * (rowH + gap2), cc._tameTab + 1);
}
var bk = new cc.Node("bk");
bk.setContentSize(140, 40);
var bkg = bk.addComponent(cc.Graphics);
bkg.fillColor = new cc.Color(90, 110, 180, 255);
bkg.roundRect(-70, -20, 140, 40, 5);
bkg.fill();
var bkl = new cc.Node("bkl");
var bkll = bkl.addComponent(cc.Label);
bkll.fontSize = 14; bkll.isBold = !0; bkll.isSystemFontUsed = !0; bkll.lineHeight = 18;
bkll.string = "返回";
bkll.color = cc.Color.WHITE;
bkl.setPosition(0, 0);
bk.addChild(bkl);
bk.setPosition(0, -H / 2 + 28);
panel.addChild(bk);
bk.on(cc.Node.EventType.TOUCH_END, function() {
cc.showBeastPanel();
});
...
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("beastTame err:" + err); }
};
cc.showBeastRune = function(mid, g) {
try {
if (cc._beastPanel && cc._beastPanel.isValid) { cc._beastPanel.destroy(); cc._beastPanel = null; }
var pd = cc.playerData;
var mc = cc.beastMonCfg();
var mcfg = mc[mid] || { name: "???" };
var panel = new cc.Node("beast_rune");
var W = 560, H = 420;
panel.setContentSize(W, H);
panel.setPosition(0, 0);
var pg = panel.addComponent(cc.Graphics);
...
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 22; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 28;
tlb.string = "驯服目标: " + mcfg.name + " ★" + (g + 1);
tlb.color = new cc.Color(255, 180, 90, 255);
t.setPosition(0, H / 2 - 30);
panel.addChild(t);
var st = new cc.Node("st");
var stl = st.addComponent(cc.Label);
stl.fontSize = 13; stl.isBold = !0; stl.isSystemFontUsed = !0; stl.lineHeight = 19;
stl.string = "魔晶: " + (pd.beastGem || 0) + "   驯兽师加成: +" + ((pd.tameLv || 0) * 5) + "%捕捉率";
stl.color = new cc.Color(255, 200, 90, 255);
st.setPosition(0, H / 2 - 58);
panel.addChild(st);
var runes = [ { n: "木符文", cost: 50, coef: 1.0, d: "基础捕捉系数" }, { n: "银符文", cost: 150, coef: 1.8, d: "捕捉x1.8" }, { n: "金符文", cost: 400, coef: 2.8, d: "捕捉x2.8" } ];
for (var ri = 0; ri < 3; ri++) {
(function(rn, xpos) {
var can = (pd.beastGem || 0) >= rn.cost;
var card = new cc.Node("rune" + ri);
card.setContentSize(160, 170);
var cg = card.addComponent(cc.Graphics);
cg.fillColor = can ? new cc.Color(40, 46, 72, 255) : new cc.Color(38, 40, 52, 255);
cg.roundRect(-80, -85, 160, 170, 8);
cg.fill();
cg.strokeColor = can ? new cc.Color(120, 220, 160, 255) : new cc.Color(70, 75, 90, 255);
cg.lineWidth = 3;
cg.roundRect(-77, -82, 154, 164, 7);
cg.stroke();
var nl = new cc.Node("nl");
var nll = nl.addComponent(cc.Label);
nll.fontSize = 16; nll.isBold = !0; nll.isSystemFontUsed = !0; nll.lineHeight = 22;
nll.string = rn.n;
nll.color = can ? cc.Color.WHITE : new cc.Color(110, 115, 130, 255);
nll.setPosition(0, 46);
card.addChild(nl);
var dl = new cc.Node("dl");
var dll = dl.addComponent(cc.Label);
dll.fontSize = 12; dll.isBold = !0; dll.isSystemFontUsed = !0; dll.lineHeight = 17;
dll.string = rn.d + "\n费用: " + rn.cost + "魔晶" + (can ? "" : "\n(魔晶不足)");
dll.color = can ? new cc.Color(180, 200, 230, 255) : new cc.Color(180, 90, 90, 255);
dl.setPosition(0, 0);
card.addChild(dl);
var gl = new cc.Node("gl");
var gll = gl.addComponent(cc.Label);
gll.fontSize = 13; gll.isBold = !0; gll.isSystemFontUsed = !0; gll.lineHeight = 17;
gll.string = "开始挑战";
gll.color = can ? new cc.Color(120, 230, 150, 255) : new cc.Color(110, 115, 130, 255);
gl.setPosition(0, -52);
card.addChild(gl);
card.setPosition(xpos, 0);
panel.addChild(card);
if (can) card.on(cc.Node.EventType.TOUCH_END, function() {
pd.beastGem -= rn.cost;
pd.saveflag = !0;
cc.tameStart(mid, ri, g);
});
})(runes[ri], (ri - 1) * 175);
}
var tips = new cc.Node("tips");
var tpl = tips.addComponent(cc.Label);
tpl.fontSize = 12; tpl.isSystemFontUsed = !0; tpl.lineHeight = 17; tpl.isBold = !0;
tpl.string = "挑战失败返还魔晶+5 · 驯兽点+5 (参与奖)";
tpl.color = new cc.Color(160, 175, 205, 255);
tips.setPosition(0, -H / 2 + 52);
panel.addChild(tips);
var cl = new cc.Node("cl");
cl.setContentSize(140, 40);
var cg2 = cl.addComponent(cc.Graphics);
...
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("beastRune err:" + err); }
};
cc.tameStart = function(mid, rune, g) {
cc.tame = { mid: mid, rune: rune, g: g, star: g, t: 60 };
cc.tameMode = !0;
cc.rogue = !1; cc.abyssMode = !1; cc.wujin = !1; cc.hell = !1; cc.mode1w = !1; cc.corridor = null;
cc.battling = !0;
cc.director.loadScene("game");
};
...
cc.tameSettle = function() {
if (!cc.tame) return;
var bo = cc.battlelogic && cc.battlelogic.bossobj;
var hpp = bo && bo.maxhp > 0 ? Math.max(0, bo.hp / bo.maxhp) : 1;
var rate = cc.tameRate(hpp);
var ok = Math.random() < rate;
cc.tameMode = !1;
cc.battling = !1;
if (ok) cc.tameSuccess(hpp); else cc.tameFail("目标挣脱了符文!");
};
cc.tameSuccess = function(hpp) {
var pd = cc.playerData;
var mid = cc.tame.mid, g = cc.tame.g || 0;
var rate = cc.tameRate(hpp);
var first = !(pd.petbook && pd.petbook[mid]);
var lv = Math.max(1, Math.floor((pd.stage || 1) * 0.8) + g * 3 - 5);
pd.catchpet(mid, lv, !1);
var gem = first ? 30 + g * 15 : 10 + g * 5;
pd.beastGem = (pd.beastGem || 0) + gem;
pd.tamePt = (pd.tamePt || 0) + (first ? 15 : 8);
pd.tameCount = (pd.tameCount || 0) + 1;
pd.saveflag = !0;
var mc = cc.beastMonCfg();
var mname = (mc[mid] || {}).name || "魔物";
cc.tame = null;
...
cc.tameMode = !1;
cc.battling = !1;
pd.beastGem = (pd.beastGem || 0) + 5;
pd.tamePt = (pd.tamePt || 0) + 5;
pd.saveflag = !0;
cc.tame = null;
cc.uiHelper.showTips(msg + " (魔晶+5 驯兽点+5)");
var sc = cc.director.getScene();
if (sc && sc.name != "main") cc.director.loadScene("main");
};
cc.showTameResult = function(ok, msg, gem, ratePct) {
try {
var panel = new cc.Node("tame_result");
var W = 520, H = 300;
panel.setContentSize(W, H);
panel.setPosition(0, 0);
var pg = panel.addComponent(cc.Graphics);
...
pg.roundRect(-W / 2, -H / 2, W, H, 10);
pg.fill();
pg.strokeColor = new cc.Color(ok ? 120 : 200, ok ? 220 : 70, ok ? 160 : 70, 255);
pg.lineWidth = 3;
pg.roundRect(-W / 2 + 3, -H / 2 + 3, W - 6, H - 6, 8);
pg.stroke();
var t = new cc.Node("t");
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 24; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 32;
tlb.string = ok ? "驯服成功!" : "驯服失败";
tlb.color = ok ? new cc.Color(120, 230, 150, 255) : new cc.Color(230, 120, 90, 255);
t.setPosition(0, 78);
panel.addChild(t);
var s = new cc.Node("s");
var slb = s.addComponent(cc.Label);
slb.fontSize = 15; slb.isBold = !0; slb.isSystemFontUsed = !0; slb.lineHeight = 22;
slb.string = msg + "\n获得魔晶+" + gem + (ok ? "" : "  (捕捉率 " + (ratePct || 0) + "%)");
slb.color = new cc.Color(200, 215, 240, 255);
s.setPosition(0, 8);
panel.addChild(s);
var ok2 = new cc.Node("ok2");
ok2.setContentSize(180, 42);
var og = ok2.addComponent(cc.Graphics);
og.fillColor = new cc.Color(60, 110, 200, 255);
og.roundRect(-90, -21, 180, 42, 6);
og.fill();
var olb = new cc.Node("olb");
var ol = olb.addComponent(cc.Label);
ol.fontSize = 15; ol.isBold = !0; ol.isSystemFontUsed = !0; ol.lineHeight = 19;
ol.string = "确定";
ol.color = cc.Color.WHITE;
olb.setPosition(0, 0);
ok2.addChild(olb);
ok2.setPosition(0, -78);
panel.addChild(ok2);
ok2.on(cc.Node.EventType.TOUCH_END, function() {
panel.destroy();
});
panel.on(cc.Node.EventType.TOUCH_START, function(e) { e.stopPropagation(); });
panel.on(cc.Node.EventType.TOUCH_MOVE, function(e) { e.stopPropagation(); });
...
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("tameResult err:" + err); }
};
setInterval(function() {
try {
if (cc.tame && cc.tameMode && cc.battling) {
cc.tame.t--;
var cv = cc.find("Canvas");
if (cv) {
if (!cc._tameTimerNode || !cc._tameTimerNode.isValid) {
cc._tameTimerNode = new cc.Node("tameTimer");
...
var hpp = bo && bo.maxhp > 0 ? Math.max(0, bo.hp / bo.maxhp) : 1;
var rate = cc.tameRate(hpp);
cc._tameTimerNode.getComponent(cc.Label).string = "驯服倒计时 " + Math.max(0, cc.tame.t) + "s | 目标血量 " + Math.round(hpp * 100) + "% | 捕捉率 " + Math.round(rate * 100) + "%";
}
if (cc.tame.t <= 0) cc.tameSettle();
} else if (cc._tameTimerNode) {
cc._tameTimerNode.isValid && cc._tameTimerNode.destroy();
cc._tameTimerNode = null;
}
} catch (e) {}
}, 1000);
cc.showBeastTeam = function() {
try {
cc.beastEnsureInit();
if (cc._beastPanel && cc._beastPanel.isValid) { cc._beastPanel.destroy(); cc._beastPanel = null; }
var pd = cc.playerData;
...
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 22; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 28;
tlb.string = "远征编队 " + pd.beastTeam.length + "/3   战力 " + cc.beastTeamPower();
tlb.color = new cc.Color(120, 230, 150, 255);
t.setPosition(0, H / 2 - 26);
panel.addChild(t);
var tips = new cc.Node("tips");
var tpl = tips.addComponent(cc.Label);
tpl.fontSize = 12; tpl.isSystemFontUsed = !0; tpl.lineHeight = 17; tpl.isBold = !0;
tpl.string = "点击宠物加入/移出编队 · 队伍均战力越高, 秘境事件成功率越高";
tpl.color = new cc.Color(160, 175, 205, 255);
tips.setPosition(0, H / 2 - 50);
panel.addChild(tips);
var mc = cc.beastMonCfg();
var per = 7;
var total = Math.ceil(pd.petbag.length / per);
for (var ri = 0; ri < per; ri++) {
var idx = cc._btPage * per + ri;
if (idx >= pd.petbag.length) break;
(function(pet, ry) {
var mcfg = mc[pet.id] || { name: "???" };
var inTeam = pd.beastTeam.indexOf(pet.uuid) >= 0;
var row = new cc.Node("pt_" + pet.uuid);
row.setContentSize(W - 40, 44);
var rg = row.addComponent(cc.Graphics);
rg.fillColor = inTeam ? new cc.Color(50, 80, 60, 255) : new cc.Color(40, 46, 72, 255);
rg.roundRect(-(W - 40) / 2, -22, W - 40, 44, 5);
rg.fill();
var nl = new cc.Node("nl");
var nll = nl.addComponent(cc.Label);
nll.fontSize = 14; nll.isBold = !0; nll.isSystemFontUsed = !0; nll.lineHeight = 19;
nll.anchorX = 0;
nll.setPosition(-(W - 40) / 2 + 14, 0);
nll.string = mcfg.name + " Lv." + (pet.lv || 1);
nll.color = cc.Color.WHITE;
row.addChild(nl);
var sl = new cc.Node("sl");
var sll = sl.addComponent(cc.Label);
sll.fontSize = 13; sll.isBold = !0; sll.isSystemFontUsed = !0; sll.lineHeight = 17;
sll.anchorX = 1;
sll.setPosition((W - 40) / 2 - 12, 0);
sll.string = inTeam ? "[已编入] 点击移出" : "点击编入";
sll.color = inTeam ? new cc.Color(120, 230, 150, 255) : new cc.Color(120, 200, 250, 255);
row.addChild(sl);
row.setPosition(0, ry);
panel.addChild(row);
row.on(cc.Node.EventType.TOUCH_END, function() {
var ix = pd.beastTeam.indexOf(pet.uuid);
if (ix >= 0) pd.beastTeam.splice(ix, 1);
else {
if (pd.beastTeam.length >= 3) { cc.uiHelper.showTips("编队已满(3只)"); return; }
pd.beastTeam.push(pet.uuid);
}
pd.saveflag = !0;
cc.showBeastTeam();
});
})(pd.petbag[idx], H / 2 - 88 - ri * 48);
}
var pv = new cc.Node("pv");
pv.setContentSize(120, 38);
var pvg = pv.addComponent(cc.Graphics);
pvg.fillColor = new cc.Color(90, 110, 180, 255);
pvg.roundRect(-60, -19, 120, 38, 5);
pvg.fill();
var pvl = new cc.Node("pvl");
var pvll = pvl.addComponent(cc.Label);
pvll.fontSize = 13; pvll.isBold = !0; pvll.isSystemFontUsed = !0; pvll.lineHeight = 17;
pvll.string = "< 上一页";
pvll.color = cc.Color.WHITE;
pvl.setPosition(0, 0);
pv.addChild(pvl);
pv.setPosition(-170, -H / 2 + 32);
panel.addChild(pv);
if (cc._btPage > 0) pv.on(cc.Node.EventType.TOUCH_END, function() {
cc._btPage--;
cc.showBeastTeam();
});
var pn = new cc.Node("pn");
pn.setContentSize(120, 38);
var png = pn.addComponent(cc.Graphics);
png.fillColor = new cc.Color(90, 110, 180, 255);
png.roundRect(-60, -19, 120, 38, 5);
png.fill();
var pnl = new cc.Node("pnl");
var pnll = pnl.addComponent(cc.Label);
pnll.fontSize = 13; pnll.isBold = !0; pnll.isSystemFontUsed = !0; pnll.lineHeight = 17;
pnll.string = (cc._btPage + 1) + "/" + Math.max(1, total) + " 下一页 >";
pnll.color = cc.Color.WHITE;
pnl.setPosition(0, 0);
pn.addChild(pnl);
pn.setPosition(170, -H / 2 + 32);
panel.addChild(pn);
if (cc._btPage < total - 1) pn.on(cc.Node.EventType.TOUCH_END, function() {
cc._btPage++;
cc.showBeastTeam();
});
var bk = new cc.Node("bk");
bk.setContentSize(140, 38);
var bkg = bk.addComponent(cc.Graphics);
bkg.fillColor = new cc.Color(200, 70, 70, 255);
bkg.roundRect(-70, -19, 140, 38, 5);
bkg.fill();
var bkl = new cc.Node("bkl");
var bkll = bkl.addComponent(cc.Label);
bkll.fontSize = 13; bkll.isBold = !0; bkll.isSystemFontUsed = !0; bkll.lineHeight = 17;
bkll.string = "返回";
bkll.color = cc.Color.WHITE;
bkl.setPosition(0, 0);
bk.addChild(bkl);
bk.setPosition(0, -H / 2 + 32);
panel.addChild(bk);
bk.on(cc.Node.EventType.TOUCH_END, function() {
cc.showBeastPanel();
});
...
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("beastTeam err:" + err); }
};
cc.expedStart = function(sid) {
cc.beastEnsureInit();
var pd = cc.playerData;
if (pd.expPower < 1) { cc.uiHelper.showTips("探索令不足(每日恢复6个)"); return; }
if (pd.beastTeam.length < 1) { cc.uiHelper.showTips("请先编队至少1只魔物"); return; }
pd.expPower--;
pd.saveflag = !0;
cc.exped = { sid: sid, deep: 0, fatigue: 0, buff: 0, gem: 0 };
cc.showExpedEvent();
};
cc.expedRollRate = function(base) {
var pd = cc.playerData;
var sc = cc.beastSancts[cc.exped.sid];
var pw = cc.beastTeamPower();
var diff = pw - sc.base;
var match = Math.max(-20, Math.min(20, diff * 2));
var r = base + match + (pd.tameLv || 0) * 2 + (cc.exped.buff || 0);
return Math.max(15, Math.min(95, r));
};
cc.showExpedEvent = function() {
try {
var pd = cc.playerData;
var sc = cc.beastSancts[cc.exped.sid];
var rep = pd.sanctRep[cc.exped.sid] || 0;
var maxDeep = 4 + rep;
...
var pg = panel.addComponent(cc.Graphics);
pg.fillColor = new cc.Color(16, 20, 40, 255);
pg.roundRect(-W / 2, -H / 2, W, H, 12);
pg.fill();
pg.strokeColor = new cc.Color(120, 220, 160, 255);
pg.lineWidth = 3;
pg.roundRect(-W / 2 + 3, -H / 2 + 3, W - 6, H - 6, 10);
pg.stroke();
var t = new cc.Node("t");
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 22; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 28;
tlb.string = sc.name + " · 深度" + cc.exped.deep + "/" + maxDeep;
tlb.color = new cc.Color(120, 230, 150, 255);
t.setPosition(0, H / 2 - 28);
panel.addChild(t);
var ft = new cc.Node("ft");
var ftl = ft.addComponent(cc.Label);
ftl.fontSize = 14; ftl.isBold = !0; ftl.isSystemFontUsed = !0; ftl.lineHeight = 20;
ftl.string = "疲劳: " + cc.exped.fatigue + "/100   本程魔晶: " + cc.exped.gem + (cc.exped.buff > 0 ? "   祝福+" + cc.exped.buff + "%成功率" : "");
ftl.color = cc.exped.fatigue >= 70 ? new cc.Color(255, 120, 90, 255) : new cc.Color(255, 200, 90, 255);
ft.setPosition(0, H / 2 - 56);
panel.addChild(ft);
var pool = [];
for (var i = 0; i < cc.beastEventPool.length; i++) pool.push(i);
var picks = [];
for (var pi = 0; pi < 3; pi++) picks.push(pool.splice(Math.floor(Math.random() * pool.length), 1)[0]);
for (var k = 0; k < 3; k++) {
(function(evt, xpos) {
var ev = cc.beastEventPool[evt];
var card = new cc.Node("eev" + evt);
card.setContentSize(215, 240);
var cg = card.addComponent(cc.Graphics);
cg.fillColor = new cc.Color(40, 46, 72, 255);
cg.roundRect(-107, -120, 215, 240, 8);
cg.fill();
cg.strokeColor = new cc.Color(120, 200, 250, 255);
cg.lineWidth = 3;
cg.roundRect(-104, -117, 209, 234, 7);
cg.stroke();
var nl = new cc.Node("nl");
var nll = nl.addComponent(cc.Label);
nll.fontSize = 16; nll.isBold = !0; nll.isSystemFontUsed = !0; nll.lineHeight = 22;
nll.overflow = cc.Label.Overflow.SHRINK;
nll.setContentSize(190, 24);
nll.string = "【" + ev.n + "】";
nll.color = new cc.Color(120, 200, 250, 255);
nll.setPosition(0, 92);
card.addChild(nl);
var dl = new cc.Node("dl");
var dll = dl.addComponent(cc.Label);
dll.fontSize = 12; dll.isBold = !0; dll.isSystemFontUsed = !0; dll.lineHeight = 17;
dl.setContentSize(190, 40);
dll.overflow = cc.Label.Overflow.SHRINK;
dll.string = ev.d;
dll.color = new cc.Color(200, 215, 240, 255);
dl.setPosition(0, 62);
card.addChild(dl);
for (var oi = 0; oi < ev.opts.length; oi++) {
(function(opt, oy) {
var rate = cc.expedRollRate(opt.base);
var ob = new cc.Node("opt" + oi);
ob.setContentSize(185, 44);
var og = ob.addComponent(cc.Graphics);
og.fillColor = new cc.Color(55, 70, 110, 255);
og.roundRect(-92, -22, 185, 44, 5);
og.fill();
var olb = new cc.Node("olb");
var oll = olb.addComponent(cc.Label);
oll.fontSize = 11; oll.isBold = !0; oll.isSystemFontUsed = !0; oll.lineHeight = 14;
oll.overflow = cc.Label.Overflow.SHRINK;
olb.setContentSize(175, 40);
oll.string = opt.t + " (" + rate + "%)";
oll.color = cc.Color.WHITE;
olb.setPosition(0, 0);
ob.addChild(olb);
ob.setPosition(0, oy);
card.addChild(ob);
...
if (opt.okBuff) cc.exped.buff = (cc.exped.buff || 0) + opt.okBuff;
if (opt.okStone) cc.playerData.additembyid(30010, 1, !0);
cc.uiHelper.showTips(opt.ok);
} else {
cc.uiHelper.showTips(opt.fail);
...
var ret = new cc.Node("ret");
ret.setContentSize(220, 42);
var rg2 = ret.addComponent(cc.Graphics);
rg2.fillColor = new cc.Color(200, 130, 60, 255);
rg2.roundRect(-110, -21, 220, 42, 6);
rg2.fill();
var rlb = new cc.Node("rlb");
var rll = rlb.addComponent(cc.Label);
rll.fontSize = 15; rll.isBold = !0; rll.isSystemFontUsed = !0; rll.lineHeight = 19;
rll.string = "携带" + cc.exped.gem + "魔晶撤离";
rll.color = cc.Color.WHITE;
rlb.setPosition(0, 0);
ret.addChild(rlb);
ret.setPosition(0, -H / 2 + 32);
panel.addChild(ret);
ret.on(cc.Node.EventType.TOUCH_END, function() {
cc.expedSettle();
});
panel.on(cc.Node.EventType.TOUCH_START, function(e) { e.stopPropagation(); });
panel.on(cc.Node.EventType.TOUCH_MOVE, function(e) { e.stopPropagation(); });
...
host.addChild(panel, 99999);
panel.setPosition(0, 0);
} catch (err) { cc.log("expedEvent err:" + err); }
};
cc.expedSettle = function() {
var pd = cc.playerData;
var sid = cc.exped.sid;
var deep = cc.exped.deep;
var gem = cc.exped.gem;
var depGain = Math.round(deep * 15 * (1 + (pd.tameLv || 0) * 0.03) * (1 + (pd.sanctRep[sid] || 0) * 0.04));
pd.beastGem = (pd.beastGem || 0) + depGain;
var repGain = Math.max(1, Math.floor(deep / 2));
var oldRep = pd.sanctRep[sid] || 0;
pd.sanctRep[sid] = Math.min(10, oldRep + repGain);
pd.sanctDeep[sid] = Math.max(pd.sanctDeep[sid] || 0, deep);
pd.saveflag = !0;
var sc = cc.beastSancts[sid];
var repUp = pd.sanctRep[sid] > oldRep;
var panel = new cc.Node("exped_r");
var W = 540, H = 340;
panel.setContentSize(W, H);
panel.setPosition(0, 0);
var pg = panel.addComponent(cc.Graphics);
...
var tlb = t.addComponent(cc.Label);
tlb.fontSize = 24; tlb.isBold = !0; tlb.isSystemFontUsed = !0; tlb.lineHeight = 32;
tlb.string = sc.name + " 探索结束";
tlb.color = new cc.Color(120, 230, 150, 255);
t.setPosition(0, 108);
panel.addChild(t);
var g = new cc.Node("g");
var glb = g.addComponent(cc.Label);
glb.fontSize = 16; glb.isBold = !0; glb.isSystemFontUsed = !0; glb.lineHeight = 24;
glb.string = "抵达深度: " + deep + "\n事件魔晶: " + gem + "\n深度奖励: " + depGain + "魔晶\n声望: +" + repGain + " (当前" + pd.sanctRep[sid] + "/10" + (repUp ? " 深度上限提升!" : "") + ")";
glb.color = new cc.Color(200, 215, 240, 255);
g.setPosition(0, 8);
panel.addChild(g);
var ok2 = new cc.Node("ok2");
ok2.setContentSize(180, 42);
