...
cc._spMake=function(){
    try{
        ...
        n.setPosition(cc.v2(sz.width/2-160,sz.height/2-40)); // 偏左
...
this.refreshdaily();
    ...
    fixdesc: function() {
        try {
            // 修改广告屋按钮描述: 双倍->5倍, 数量文字
            var arr = this.node.children || [];
            var i, node, lb;
            for (i = 0; i < arr.length; i++) {
                node = arr[i];
                if (!node) continue;
                lb = node.getComponent(cc.Label);
                if (!lb) continue;
                var s = lb.string;
                if (s.indexOf("双倍") !== -1) lb.string = s.replace(/双倍/g, "5倍");
                else if (s.indexOf("翻倍") !== -1) lb.string = s.replace("翻倍", "5倍");
                else if (s.indexOf("五个重铸石") !== -1) lb.string = "获得一百个重铸石";
                else if (s.indexOf("五个洗练卷") !== -1) lb.string = "获得一百个洗练卷";
                else if (s.indexOf("盲盒一个") !== -1) lb.string = "盲盒十个";
                else if (s.indexOf("武器技能水") !== -1) lb.string = s.replace("+5武器技能水", "+100武器技能水");
            }
            // 金币描述
            if (this.lb_gold) this.lb_gold.string = "获得" + this.goldv + "金币";
        } catch (e) {}
    },
tttt: function() {
...
onmofaitem: function() {
this.ongetitem(20509, 100);
},
onvititem: function() {
this.ongetitem(20502, 50);
},
onmanghe: function() {
